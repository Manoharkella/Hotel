...
- *(Optional)* **PostgreSQL**: Version 14+ if using a local PostgreSQL database instead of SQLite.
...
- **On Windows (PowerShell):**
  ...
  ```
  *(If PowerShell displays an Execution Policy error, run: `Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass` then re-run the activate command)*
...
#### C. *(Optional)* Configure Environment
